...
class DaftarPengurusController extends GetxController {
  static DaftarPengurusController get instance => Get.find();
  RxList<dynamic> data = <dynamic>[].obs;
  Rx<List<Map<String, dynamic>>> pengurusList =
  ...
  void fetchAllPengurus() async {
    try {
      final response = await SupBase.supabase
          .from('data_anggota')
          .select()
          .neq('user_id', AuthenticationRepository.instance.authUser!.id)
          .eq('user_role', 'Pengurus')
          .eq('status', 'Aktif');
      pengurusList.value = response;
    } catch (e) {
      rethrow;
    }
  }

  void searchPengurus(String searchText) async {
    try {
      final response = await SupBase.supabase
          .from('data_anggota')
          .select()
          .or('fullname.ilike.%$searchText%,no_anggota.ilike.%$searchText%')
          .or('user_role.eq.Pengurus,user_role.eq.Super Admin')
          .eq('status', 'Aktif')
          .neq('user_id', AuthenticationRepository.instance.authUser!.id);
      pengurusList.value = response;
    } catch (e) {
  ...
  void nonAktifPengurus(String idPengurus) async {
    try {
      await SupBase.supabase
          .from('user_data')
          .update({'role': 'Anggota'}).match({'user_id': idPengurus});
  ...
  void ubahStatus(String peran, String idPengurus) async {
    try {
      await SupBase.supabase
          .from('data_anggota')
          .update({'user_role': peran}).eq('user_id', idPengurus);
      await SupBase.supabase
          .from('user_data')
          .update({'role': peran}).eq('user_id', idPengurus);
      fetchAllPengurus();
    } catch (e) {
      throw '$e';
    }
  ...
  Future<void> createAndDisplayPDF(BuildContext context) async {
    ...
    final pageCount = (pengurusList.value.length / maxCardsPerPage).ceil();
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (pw.Context context) {
            ...
            for (var anggota in pengurusList.value.sublist(
                startIndex,
                endIndex < pengurusList.value.length
                    ? endIndex
                    : pengurusList.value.length)) {
  ...
  void deactivatePengurus(String idPengurus) async {
    try {
      try {
        await SupBase.supabase
            .from('data_anggota')
            .update({'status': 'Non-Aktif'}).match({'user_id': idPengurus});
        fetchAllPengurus();
        Get.back();
      } catch (e) {
        rethrow;
